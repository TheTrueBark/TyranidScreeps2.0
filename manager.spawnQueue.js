    ...
    if (sortedQueue.length > 0) {
      ...
      if (!nextSpawn.memory) {
        statsConsole.log(
          ...
          3,
          { module: 'spawnQueue', room: Game.getObjectById(spawnId).room.name },
        );
      }
      return nextSpawn;
    }
    ...
    if (index !== -1) {
      ...
      if (debugConfig.spawnQueue)
        statsConsole.log(
          ...
          2,
          { module: 'spawnQueue', room: Game.getObjectById(removed.spawnId).room.name },
        );
      return removed;
    }
    return null;
  ...
  adjustPriorities(room) {
    if (debugConfig.spawnQueue)
      statsConsole.log(
        ...
        2,
        { module: 'spawnQueue', room: room.name },
      );
      ...
      if (nextSpawn && spawn.room.energyAvailable >= nextSpawn.energyRequired) {
        ...
        if (debugConfig.spawnQueue)
          statsConsole.log(
            ...
            2,
            { module: 'spawnQueue', room: spawn.room.name },
          );
