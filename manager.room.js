const roomManager = {
    ...
    scanRoom: function(room) {
        ...
        sources.forEach(source => {
            ...
            const miningSpots = potentialMiningSpots.filter(spot => room.getTerrain().get(spot.x, spot.y) !== TERRAIN_MASK_WALL);

            // Sort mining spots by distance to spawn
            miningSpots.sort((a, b) => spawn.pos.getRangeTo(a.x, a.y) - spawn.pos.getRangeTo(b.x, b.y));
            ...
            Memory.rooms[room.name].miningPositions[source.id] = {
                ...
                y: sourcePos.y,
                positions: {
                    best1: bestPositions[0] ? { ...bestPositions[0], reserved: false } : null,
                    best2: bestPositions[1] ? { ...bestPositions[1], reserved: false } : null,
                    best3: bestPositions[2] ? { ...bestPositions[2], reserved: false } : null
                }
            };
        });
