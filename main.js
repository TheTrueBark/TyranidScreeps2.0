...
const debug = require("console.debugLogs");
...
// expose debug configuration controls to the console
global.debug = {
  toggle: debug.toggle,
  list: debug.list,
  setThreshold: debug.setThreshold,
};
...
scheduler.addTask("clearMemory", 100, () => {
  for (let name in Memory.creeps) {
    if (!Game.creeps[name]) {
      statsConsole.log(
        `Clearing memory of dead creep: ${name}`,
        2,
        { module: 'main', task: 'clearMemory' },
      );
