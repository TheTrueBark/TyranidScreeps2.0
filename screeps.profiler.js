...
function getFilter() {
  return Memory.profiler && Memory.profiler.filter;
}
...
const Profiler = {
  ensureMemory() {
    if (!Memory.profiler || typeof Memory.profiler !== 'object') {
      Memory.profiler = {
        map: {},
        totalTime: 0,
    ...
    if (!Memory.profiler.map || typeof Memory.profiler.map !== 'object') {
      Memory.profiler.map = {};
    }
    if (typeof Memory.profiler.totalTime !== 'number') {
      Memory.profiler.totalTime = 0;
    }
    if (typeof Memory.profiler.enabledTick !== 'number') {
      Memory.profiler.enabledTick = Game.time;
    }
  ...
  checkMapItem(functionName, map) {
    const profiler = this.ensureMemory();
    const resolvedMap = map || profiler.map;
    if (!resolvedMap) return;
    if (!resolvedMap[functionName]) {
      // eslint-disable-next-line no-param-reassign
      resolvedMap[functionName] = {
        time: 0,
        calls: 0,
        subs: {},
      };
  ...
  record(functionName, time, parent) {
    ...
    profiler.map[functionName].calls++;
    profiler.map[functionName].time += time;
    if (parent) {
      this.checkMapItem(parent);
      this.checkMapItem(functionName, profiler.map[parent].subs);
      profiler.map[parent].subs[functionName].calls++;
      profiler.map[parent].subs[functionName].time += time;
    }
  },

  endTick() {
    if (Memory.profiler && Game.time >= Memory.profiler.enabledTick) {
