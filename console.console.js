/**
 * Stats and Logging Console
 * -------------------------
 * This module renders CPU statistics and custom logs directly in the Screeps
 * console. Use `statsConsole.log(message, severity, context)` to record
 * messages. Each log entry stores a severity level (0–5) and optional context
 * information like module, colony, room or task. Messages with the same text
 * are counted and automatically raise their severity every 10 occurrences to
 * highlight persistent issues.
 *
 * A global severity filter can be configured via `Memory.logConfig`.
 * Only logs with a severity greater than or equal to
 * `Memory.logConfig.severityThreshold` (defaults to 3) will be displayed.
 */
...
var statsConsole = {
  run: function (data, logCpu = true, opts = {}) {
    ...
    if (!Memory.logConfig) {
      Memory.logConfig = { severityThreshold: 3 };
    }
  ...
  /**
   * Store a log message in Memory.stats.logs
   * @param {string} message  - Text to store
   * @param {number} severity - 0 (lowest) to 5 (highest)
   * @param {Object} context  - Optional context information
   *                            {module, colony, room, task}
   */
  log: function (message, severity = 3, context = {}) {
    if (!Memory.logCounts) Memory.logCounts = {};
    if (!Memory.stats.logs) Memory.stats.logs = [];

    // Count repeated messages and raise severity every 10 occurrences
    const count = (Memory.logCounts[message] || 0) + 1;
    Memory.logCounts[message] = count;
    if (count % 10 === 0) {
      severity = Math.min(severity + 1, 5);
    }

    // Build a prefix from context information
    let prefix = "";
    if (context.colony) prefix += `[${context.colony}]`;
    if (context.room) prefix += `[${context.room}]`;
    if (context.task) prefix += `[${context.task}]`;
    if (context.module) prefix += `[${context.module}]`;

    Memory.stats.logs.push([`${Game.time}: ${prefix} ${message}`, severity]);
  },
  ...
  displayLogs: function (logs = Memory.stats.logs, opts = {}) {
    ...
    const threshold =
      (Memory.logConfig && Memory.logConfig.severityThreshold) || 3;
    logs = logs.filter((l) => l[1] >= threshold);
