    ...
    init() {
        Creep.prototype.registerMove = function (target) {
            ...
            if (debugConfig.trafficManager) {
                console.log(`Creep ${this.name} registered move to (${targetPosition.x}, ${targetPosition.y}) with packedCoord ${packedCoord}`);
            }
        };
    ...
    run(room, costs, threshold) {
        ...
        for (const creep of creepsWithMovementIntent) {
            if (creep._matchedPackedCoord === creep._intendedPackedCoord) {
                if (debugConfig.trafficManager) {
                    console.log(`Creep ${creep.name} already at intended position (${unpackCoordinates(creep._matchedPackedCoord).x}, ${unpackCoordinates(creep._matchedPackedCoord).y})`);
                }
                continue;
            }
        ...
        for (const creep of creepsInRoom) {
            ...
            if (creep.pos.isEqualTo(matchedPosition.x, matchedPosition.y)) {
                if (debugConfig.trafficManager) {
                    console.log(`Creep ${creep.name} is already at position (${matchedPosition.x}, ${matchedPosition.y})`);
                }
                continue;
            }
            ...
            if (debugConfig.trafficManager) {
                console.log(`Creep ${creep.name} moving from (${creep.pos.x}, ${creep.pos.y}) to (${matchedPosition.x}, ${matchedPosition.y}) in direction ${direction}`);
            }
            ...
            if (moveResult !== OK) {
                if (debugConfig.trafficManager) {
                    console.log(`Creep ${creep.name} failed to move in direction ${direction} with error ${moveResult}`);
                }
            } else {
                if (debugConfig.trafficManager) {
                    console.log(`Creep ${creep.name} successfully moved to direction ${direction}`);
                }
            }
        }
    },
    ...
    for (const coord of [...emptyTiles, ...occupiedTiles]) {
        ...
        if (!occupyingCreep) {
            if (score > 0) {
                ...
                if (debugConfig.trafficManager) {
                    console.log(`Creep ${creep.name} assigned to new position (${coord.x}, ${coord.y})`);
                }
            }
            return score;
        }
        ...
        if (!visitedCreeps[occupyingCreep.name]) {
            ...
            if (result > 0) {
                assignCreepToCoordinate(creep, coord);
                if (debugConfig.trafficManager) {
                    console.log(`Creep ${creep.name} re-assigned to position (${coord.x}, ${coord.y}) after DFS`);
                }
                return result;
            }
        }
