...
const spawnManager = {
  ...
  run(room) {
    if (debugConfig.spawnManager)
      statsConsole.log(`Running spawnManager for room: ${room.name}`,
        2,
        { module: 'spawnManager', room: room.name });
  ...
  spawnMiner(spawn, room, energyCapacityAvailable) {
    ...
    if (!Array.isArray(sources)) {
      statsConsole.log(
        `No sources found in room ${room.name}`,
        2,
        { module: 'spawnManager', room: room.name },
      );
      return;
  ...
  processSpawnQueue(spawn) {
    if (debugConfig.spawnManager)
      statsConsole.log(
        ...
        2,
        { module: 'spawnManager', room: spawn.room.name },
      );
    if (!spawn.spawning) {
